...
| `GET` | `/v1/speech` | Last Mac-synthesized reply WAV when `reply_sink` is `phone` (`/v1/tts` alias). Bearer same as `/v1/screen`. 15s timeout. 404 = none yet |
| `POST` | `/v1/command` | `{ "text": "…", "sink": "phone" }` — optional `sink` (`phone` \| `mac`). App sends `sink: "phone"` only when **Reply on phone** is on |
| `POST` | `/v1/audio` | Hold-to-talk clip → Mac STT → command queue; optional `sink` in JSON. App adds `sink: "phone"` only when the toggle is on |
| `POST` | `/v1/photo` | Camera still → Jarvis looks at it; optional `sink` in JSON. App adds `sink: "phone"` only when the toggle is on |
...
TTS stays on the Mac. Home/Settings **Reply on phone** (default on) adds `"sink": "phone"` to `/v1/command`, `/v1/audio`, and `/v1/photo`. When off, sink is omitted and the Mac keeps its current `reply_sink`. When `speech_at` changes the app refetches `/v1/speech` and plays the WAV locally. Mac wake-word turns use the Mac speakers and do not publish that file.
...
- [ ] **speech** — With **Reply on phone** on, after command/audio/photo Last spoken shows PHONE then PLAYING; WAV from `GET /v1/speech` plays on the phone. Toggle off → no `sink` field; Mac speakers keep the current sink. Mac wake-word replies stay on the Mac and do not update `speech_at`.
